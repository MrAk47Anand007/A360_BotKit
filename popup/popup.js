...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let autosaveSettings = DEFAULT_AUTOSAVE_SETTINGS;
    let autosaveStatusPoll = null;
    ...
    function startAutosaveBadgePolling() {
        if (autosaveStatusPoll) {
            clearInterval(autosaveStatusPoll);
        }

        refreshAutosaveBadge();
        autosaveStatusPoll = setInterval(refreshAutosaveBadge, 1500);
    }

    function renderAutosaveSettings() {
        const delaySeconds = Math.floor(autosaveSettings.delayMs / 1000);
        const modeLabel = autosaveSettings.mode === 'silent' ? 'Silent' : 'Native';

        elements.autosaveToggle.checked = autosaveSettings.enabled;
        elements.autosaveDelay.value = String(autosaveSettings.delayMs);
        elements.autosaveMode.value = autosaveSettings.mode;
        elements.autosaveFallback.checked = autosaveSettings.fallbackToNative;
        elements.autosaveDelay.disabled = !autosaveSettings.enabled;
        elements.autosaveMode.disabled = !autosaveSettings.enabled;
        elements.autosaveFallback.disabled = !autosaveSettings.enabled || autosaveSettings.mode !== 'silent';
        elements.autosaveStatus.textContent = autosaveSettings.enabled
            ? autosaveSettings.mode === 'silent'
                ? `Auto save is on. Silent mode runs after ${delaySeconds} seconds and ${autosaveSettings.fallbackToNative ? 'can' : 'will not'} fall back to native Save.`
    ...
    async function persistAutosaveSettings(nextSettings) {
        autosaveSettings = normalizeAutosaveSettings(nextSettings);
        renderAutosaveSettings();

        try {
            await saveAutosaveSettings(autosaveSettings);
            refreshAutosaveBadge();
        } catch (error) {
    ...
    (async () => {
        autosaveSettings = await loadAutosaveSettings();
        renderAutosaveSettings();
